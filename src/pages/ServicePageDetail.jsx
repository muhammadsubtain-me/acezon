...
export default function ServicePageDetail({ serviceId: propServiceId }) {
  ...
  const [activeDomainId, setActiveDomainId] = useState('mechanical');
  ...
  const handleTabChange = (domainId) => {
    setFade(false);
    setTimeout(() => {
  ...
  // Reset to first domain on service change
  useEffect(() => {
    setActiveDomainId('mechanical');
  }, [serviceId]);

  const currentContent = contentData[activeDomainId]?.[service.id] || {
      ...
      <section className="py-20 bg-[var(--color-section-alt)]">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-[var(--color-text-muted)] mb-8">
            Connect with our verified {activeDomain.name} experts today and get the academic help you deserve.
          </p>
          <Button size="lg" asChild>
            <Link to="/contact" className="inline-flex items-center gap-2">
